...
while true; do
    if tmux has-session -t node-i 2>/dev/null; then
        ...
        if curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:${PORT}/health" 2>/dev/null | grep -q "200\|404"; then
            echo "[12:34:34] node-i: ONLINE (port ${PORT})"
        else
            echo "[12:34:34] node-i: UNRESPONSIVE (port ${PORT})"
        fi
    else
        echo "[12:34:34] node-i: TMUX SESSION MISSING"
    fi
    sleep 10
done
